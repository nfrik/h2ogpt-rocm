...
RUN python3.10 -m pip install torch==2.0.0+rocm5.4.2 torchvision==0.15.1+rocm5.4.2 torchaudio==2.0.1 --index-url https://download.pytorch.org/whl/rocm5.4.2
RUN python3.10 -m pip install -r requirements.txt
COPY . .

WORKDIR /workspace/bitsandbytes-rocm
RUN make hip
RUN CUDA_VERSION=gfx908 python3.10 setup.py install
WORKDIR /workspace

#RUN pip install torch==2.0.0+rocm5.4.2 torchvision==0.15.1+rocm5.4.2 torchaudio==2.0.1 --index-url https://download.pytorch.org/whl/rocm5.4.2

RUN python3.10 -m pip install --upgrade setuptools
RUN python3.10 -m pip install --ignore-installed apscheduler
